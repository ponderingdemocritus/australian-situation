...
pub async fn run(
    ...
    tracing::info!("sync-lending-quarterly: starting");
    let points = aus_sources::au::abs::fetch_lending(client).await?;
    let observations =
    ...
    tracing::info!("sync-lending-quarterly: upserted {inserted} observations");
    Ok(())
}
