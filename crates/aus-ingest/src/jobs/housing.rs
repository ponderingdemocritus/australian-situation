...
pub async fn run(
    ...
    tracing::info!("sync-housing-daily: starting");
    let points = aus_sources::au::abs::fetch_housing(client).await?;
    let observations =
    ...
    tracing::info!("sync-housing-daily: upserted {inserted} observations");
    Ok(())
}
