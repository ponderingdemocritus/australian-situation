...
pub async fn run(
    ...
    tracing::info!("sync-energy-nem-mix-5m: starting");
    let url = super::source_url("aemo_nem_source_mix");
    let points = aus_sources::au::aemo_nem_mix::fetch_nem_mix(client, &url).await?;
    let now = Utc::now();
    let observations: Vec<LiveObservation> = points
        .into_iter()
        .map(|p| LiveObservation {
            series_id: format!("energy.nem_mix.{}", p.source_key),
    ...
    tracing::info!("sync-energy-nem-mix-5m: upserted {inserted} observations");
    Ok(())
}
