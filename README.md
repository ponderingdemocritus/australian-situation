...
| `/api/energy/live-wholesale` | `region` (default `AU`), `window` (`5m`/`1h`/`24h`) | `energy.wholesale.rrp.au_weighted_aud_mwh` (AU) or `energy.wholesale.rrp.region_aud_mwh` (state, with AU fallback) | Returns latest + rollups + freshness; `isModeled` is `true` when a state falls back to AU data |
| `/api/energy/retail-average` | `region` (default `AU`), `customer_type` | `energy.retail.offer.annual_bill_aud.mean` + `.median` (region, then AU fallback) | `customer_type` is passed through as `customerType`; `isModeled` is `true` when a region falls back to AU data; when no retail observations exist, `freshness.updatedAt` falls back to `1970-01-01` and status is `stale` |
...
| `/api/metadata/sources` | None | Source catalog (`sourceId`, `domain`, `name`, `url`, `expectedCadence`) | Canonical source registry exposed consistently across store and Postgres backends |
